...
def preprocess_ml_data(data, split, target='target'):
...
def discriminative_validation(real_df, synthetic_df, classifier_list, split=0.8):
    ...
    # Preprocess data
    x_train, y_train, x_test, y_test = preprocess_ml_data(mixed_data, split)
...
def preprocess_sa_data(concat_df, real_n_samples):
    # Separate real and synthetic data
    real_df = concat_df[:real_n_samples]
    syn_df = concat_df[real_n_samples:]
...
def survival_analysis(real_df, synthetic_df):
    ...
    real_x, real_y, syn_x, syn_y = preprocess_sa_data(concat_df, real_df.shape[0])
...
def utility_validation(real_df, synthetic_df):
    # Imbalance correction. We need to compare same size datasets
    real_df, synthetic_df = fix_imbalance(real_df, synthetic_df)

    results = survival_analysis(real_df, synthetic_df)

    return results


# ------------------------------------------------------------------------------------------------------
#                                   DIVERGENCES VALIDATION
# ------------------------------------------------------------------------------------------------------


def divergences_validation(real_df, synthetic_df, device='cpu'):
    # Imbalance correction. We need to compare same size datasets
    real_df, synthetic_df = fix_imbalance(real_df, synthetic_df)

    # Dataframe to tensor
    real_data = torch.tensor(real_df.values, dtype=torch.float32, device=device)
    synthetic_data = torch.tensor(synthetic_df.values, dtype=torch.float32, device=device)

    # Divergence estimation
    # estimate_divergences(real_data, synthetic_data)

    pass  ## TODO: Implement divergence estimation
