...
from src.base_model.vae_model import VariationalAutoencoder
from statsmodels.stats.proportion import proportion_confint
from src.base_model.vae_utils import check_nan_inf, sample_from_dist
from src.base_model.vae_modules import Decoder, LogLikelihoodLossWithCensoring
...
class SAVAE(VariationalAutoencoder):
    ...
    def fit_epoch(self, data, optimizer, batch_size=64, device=torch.device('cpu'), no_drop_col_stop=None):
        epoch_results = {'loss_tr': 0.0, 'loss_va': 0.0,
                         'kl_tr': 0.0, 'kl_va': 0.0,
                         'll_cov_tr': 0.0, 'll_cov_va': 0.0,
                         'll_time_tr': 0.0, 'll_time_va': 0.0}
        cov_train, mask_train, time_train, censor_train, cov_val, mask_val, time_val, censor_val = data

        # Train epoch
        if no_drop_col_stop is not None:
            cov, mask, time, censor = no_drop_col_stop
            cov_val = torch.from_numpy(cov).to(device).float()
            mask_val = torch.from_numpy(mask).to(device).float()
            time_val = torch.from_numpy(time).to(device).float()
            censor_val = torch.from_numpy(censor).to(device).float()
        else:
    ...
    def fit(self, data, train_params, save_best_model=True, no_col_drop_test_data=None, no_col_drop_stop=False):
        training_stats = {'loss_tr': [], 'loss_va': [],
                          'kl_tr': [], 'kl_va': [],
                          'll_cov_tr': [], 'll_cov_va': [],
                          'll_time_tr': [], 'll_time_va': [],
                          'ci_va': [], 'ibs_va': [],
                          'no_col_drop_ci_va': [], 'no_col_drop_ibs_va': []}

        optimizer = torch.optim.Adam(self.parameters(), lr=train_params['lr'])

        self.early_stopper.reset()  # TODO: Juan added this, needed to reset the early stopper for each local training phase
        ...
        for epoch in range(train_params['n_epochs']):
            ...
            if no_col_drop_test_data is not None:
                no_col_drop_cov_val, no_col_drop_mask_val = no_col_drop_test_data
                no_col_drop_time_val = np.array(no_col_drop_cov_val.loc[:, 'time'])
                no_col_drop_censor_val = np.array(no_col_drop_cov_val.loc[:, 'event'])
                no_col_drop_cov_val = np.array(no_col_drop_cov_val.drop(['time', 'event'], axis=1))
                no_col_drop_mask_val = np.array(no_col_drop_mask_val.drop(['time', 'event'], axis=1))
                epoch_results = self.fit_epoch(ep_data, optimizer, batch_size=train_params['batch_size'],
                                               device=train_params['device'], no_drop_col_stop=(
                    no_col_drop_cov_val, no_col_drop_mask_val, no_col_drop_time_val, no_col_drop_censor_val))
            ...
            if no_col_drop_test_data is not None:
                no_col_drop_val, _ = no_col_drop_test_data
                no_col_drop_censor_val = np.array(no_col_drop_val.loc[:, 'event'])
                epoch_results['no_col_drop_ci_va'], epoch_results['no_col_drop_ibs_va'] = self.calculate_risk(
                    time_train, no_col_drop_val, no_col_drop_censor_val)
                epochs_no_col_drop_ci.append(epoch_results['no_col_drop_ci_va'])
                epochs_no_col_drop_ibs.append(epoch_results['no_col_drop_ibs_va'])
            ...
            if epoch % 50 == 0:
                print('Iteration = ', epoch,
                      ...
                      '; C-index (no col drop) = ', '{:.4f}'.format(np.mean(
                        epoch_results['no_col_drop_ci_va'])) if no_col_drop_test_data is not None else 'Not available')
            ...
            if save_best_model:
                if epoch_results['loss_va'] < best_loss:
                    ...
                    if no_col_drop_test_data is not None:
                        best_no_col_drop_ci = epoch_results['no_col_drop_ci_va']
                        best_no_col_drop_ibs = epoch_results['no_col_drop_ibs_va']

            if self.early_stop and self.early_stopper.stop:
                print('[INFO] Training early stop on epoch' + str(epoch))
                break
        ...
        if save_best_model:
            ...
            if no_col_drop_test_data is not None:
                training_stats['no_col_drop_ci_va'].append(best_no_col_drop_ci)
                training_stats['no_col_drop_ibs_va'].append(best_no_col_drop_ibs)
